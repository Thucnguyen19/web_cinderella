          ...
          <form action="{{ route('users.store') }}" method="post" >
            ...
            {{-- form trong laravel bat buoc phai co @csrf để submit --}}
              <div class="mb-3">
                ...
                <input type="text" name="name" value="{{ old('name') }}"  class="form-control  @error('name') is-invalid @enderror" placeholder="Nhập tên user ">
                @error('name')
              ...
              <div class="mb-3">
                ...
                <input type="password" name="password" value="{{ old('password') }}"  class="form-control  @error('password') is-invalid @enderror" placeholder="Nhập tên password ">
                @error('password')
               <div class="alert alert-danger error_message">{{ $message }}</div>
               @enderror
              </div>
              <div class="mb-3">
                ...
                <select name="role_id[]" class="form-control select2_init" multiple id="">
                  ...
                  @foreach ($roles as $role )
                    <option class="select_role" value="{{ $role->id }}">{{ $role->name }}</option>
                  @endforeach
